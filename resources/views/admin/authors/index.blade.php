...
@section('content')
<a class="btn btn-primary" href="{{ route('authors.create') }}" style="color: rgb(255, 255, 255); text-decoration: none;"><i class="fas fa-plus"></i> أضف مؤلفًا جديدًا</a>
<hr>
<div class="row">
    <div class="col-md-12">
        <table id="books-table" class="table table-striped table-bordered text-right" width="100%" cellspacing="0">
            ...
            <tbody>
                @foreach($authors as $author)
                    <tr>
                        <td>{{ $author->name }}</td>
                        <td>{{ Str::limit($author->body , 30) }}</td>
                        <td>
